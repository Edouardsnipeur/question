/**
 * Created by Edouard on 11/11/2018.
 */
! function(e, t) {
...
! function(a) {
    a.fn.toggleAttr = function(attr, val) {
...
//Modifier
        function t() {
            for (var e = m.useMultipliers, t =  0, n = 0; n < j.length; n++) {

                if ((e==true)&&(j[n].plus==true)){
                    if (n==0){
                        t=j[n].pricePerHour;
                    }else {
                        t+=j[n].pricePerHour;
                    ...
                    if (e==true){
                        ...
                        t *= j[n].answers[0].answerWeight;
                    }else {
                        t += j[n].answers[0].answerWeight * m.pricePerHour
                    }
                }
            }
        ...
        function o() {
            a(),d.initial.fadeIn(600)
        }

        function r() {
            a(), d.results.fadeIn(600), $(".js--form-questions").val(t()), $(".js--form-estimate").val(t())
        }

        function i(e) {
            a(), n(), d.questions.filter('[data-question-id="' + e + '"]').fadeIn(600)
        }
        ...
        function q(e, t) {
            ...
            for (var i = 0; i  < t.length; i++){
                o.push({
                    answerId:t[i].data("answer-id"),
                    answerText: t[i].text().trim(),
                    answerWeight: t[i].data("answer-weight")
                })
            }
            ...
            if (e.data('pph')){
                for (var i =0; i  < o.length; i++){
                    h=h+o[i].answerWeight;
                }
            }
            return e.data("pph") && (n.pricePerHour = h,n.plus=true), n
        }

        //modification
        function w(t, n) {
            var a = d.questions.filter('[data-question-id="' + n + '"]'),o=[],r;
            for (var i=0; i<t.length; i++){

                o.push(a.find(".answer").filter('[data-answer-id="' + t[i] + '"]'));
            }
                r = q(a, o);

            if (o.length==1){
                a.find(".answer").removeClass("selected"), o[0].addClass("selected")
            }
             e() || !h(r.questionId) ? v(r) : g(r);
        ...
        function E() {
            for (var e = $("#devis"), r = "", i = 0; i < j.length; i++) {
                var s = j[i];

                if (s.answers.length==1){

                    r += "<li>", r += " " + s.questionId + " - " + s.questionText + ":</br>",
                        r += "<b>" + s.answers[0].answerText + "</b> <span>"+s.answers[0].answerWeight+""+m.currency.symbol+"</span>", r += "</li>"
                }else {
                    for (var u="",  z = 0; z < s.answers.length; z++) {
                        u+="<li>", u +="<b>" + s.answers[z].answerText + "</b> <span>"+s.answers[z].answerWeight+""+m.currency.symbol+"</span>", r += "</li>";
                    }
                    r += "<li>", r += " " + s.questionId + " -" + s.questionText + ":</br>",
                        r += "<ul>"+u+"</ul>", r += "</li>"
                }
            ...
            r = "<ul>" + r + "</ul><p>Le prix de total de votre projet est: <span class='total'>"+ t()+""+m.currency.symbol+"</span></p>";


            e.find('.formulaire').val(r);
            e.submit();
    ...
    function a(e) {
        var t = $(e.currentTarget),
            n = t.data("answer-id"),
            l=t.closest(".question"),
            a = l.data("question-id"),
            q = t.closest(".question").data("qcm");
            if (q==1){
                var a = d.questions.filter('[data-question-id="' + a + '"]'),
                    o = a.find(".answer").filter('[data-answer-id="' + n + '"]');
                    o.toggleClass('selected');
                r = l.find('.answer').filter('.selected');
                if (r.length>0){
                    l.find('.js--suivant').css({
                        'display':'block',
                    });
                }else {
                    l.find('.js--suivant').css({
    ...
    function S(e) {
        var t = $(e.currentTarget),
            ...
            o = t.closest(".question").find('.answer').filter('.selected'),
        n=[];
        o.each(function () {
            n.push($(this).data("answer-id"));
        });
        f.selectAnswer(n, a);
    ...
    function D() {
        ...
        e.find('.command').val('true'),u();
    }
    function E() {
        console.log('bonjour');
        var e = $("#devis");

        e.find('.formulaire').val('false'),u();
    }
    ...
    var d = {
        ...
        m = e(),
        f = new t;
    d.initial.on("click", ".js--start", n),d.all.on("click",".js--suivant",S), d.questions.on("click", ".js--answer", a), d.questions.on("click", ".js--previous", r), d.results.on("click", ".js--change", o), d.results.on("click", ".js--restart", i), d.results.on("click", ".js--toggle-results", s), d.results.on("click", ".js--start-project",E, u), d.results.on("click", ".js--command",D), $("#quoteForm").on("click", ".js--form-send", l), c();
}), window.onload = init;
